...
def run(username: str, password: str, member_id: str, acct_type: str, deposit: str) -> dict:
    ...
    with sync_playwright() as p:
        ...
        assert f'/home' in page.url, 'checkpoint failed at step 2'
        # step 3: Fill in the Member ID field with the provided member_id parameter to search for member 12345
        ctx = page
        ctx.locator("xpath=" + '//tr[td[normalize-space(.)="Member ID"]]//input').first.fill(member_id)
        # step 4: Click the Search button to search for member 12345 whose ID has already been filled in from the previous step.
        ctx = page
        ctx.get_by_role('button', name='Search').first.click()
        assert f'/member?mid={member_id}' in page.url, 'checkpoint failed at step 4'
        # step 5: Click the "Open New Sub-Account" link to begin creating a new Money Market sub-account for member 12345.
        ctx = page
        ...
        # step 6: Select 'Money Market' as the account type from the dropdown menu using the provided acct_type parameter.
        ctx = page
        ctx.locator("xpath=" + '//tr[td[normalize-space(.)="Account Type"]]//select').first.select_option(label=acct_type)
        # step 7: Fill in the Initial Deposit field with the provided deposit amount of 500.00
        ctx = page
        ctx.locator("xpath=" + '//tr[td[normalize-space(.)="Initial Deposit"]]//input').first.fill(deposit)
        # step 8: Click the Review button to proceed to the review screen for the new sub-account with the already-filled details (Money Market account type and 500.00 initial deposit).
        ctx = page
        ctx.get_by_role('button', name='Review').first.click()
        assert f'/subaccount/review' in page.url, 'checkpoint failed at step 8'
        ...
        assert f'/subaccount/confirm' in page.url, 'checkpoint failed at step 9'
