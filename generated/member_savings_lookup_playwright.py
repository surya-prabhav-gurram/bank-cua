...
def run(username: str, password: str, member_id: str) -> dict:
    ...
    with sync_playwright() as p:
        ...
        # step 0: Fill in the username field with the provided operator credentials
        ctx = page
        ...
        # step 3: Enter the member ID 12345 into the search field to search for the member's account
        ctx = page
        ctx.locator("xpath=" + '//tr[td[normalize-space(.)="Member ID"]]//input').first.fill(member_id)
        # step 4: Submit the member search form to retrieve member 12345's account information
        ctx = page
        ...
        # step 6: Extract the savings balance as required by the goal. The balance is $4,213.55 which needs to be normalized to integer cents (421355).
